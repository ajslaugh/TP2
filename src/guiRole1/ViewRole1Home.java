...
public class ViewRole1Home {
	...
	// GUI ARea 2: Invites user to create a post hw2
	...
	// GUI Area 4: This is last of the GUI areas.  It is used for quitting the application and for
	// logging out. 
	...
	public static void displayRole1Home(Stage ps, User user) {
		...
		theStage.setTitle("CSE 360 Foundations: Student Home Page"); // ***MODIFIED*** Role1 -> Student									
	...
	private ViewRole1Home() {
		...
		// GUI Area 1
		label_PageTitle.setText("Student Home Page"); // ***MODIFIED***
		...
		postDisplay = theDatabase.displayPostsHelper();
		...
		displayPosts.setOnMouseClicked(event -> {
		    if (event.getClickCount() == 2) {
		        ...
		        if (selected >= 0) {
		        	if(theDatabase.isOwnPost(selected)) {
		        		if(ControllerRole1Home.deletePost(selected)) {
		        			postDisplay.setAll(theDatabase.displayPostsHelper());
		        		}
		        	}
		        	
		            ControllerRole1Home.viewReplies(selected);
		        }
		    }
		});
